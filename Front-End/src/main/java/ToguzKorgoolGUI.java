...
public class ToguzKorgoolGUI extends JFrame implements ActionListener{
    ...
    private void setUpHoles() {
        ...
        //computer
        for(int i =0; i < MAX_HOLES; i++) {
            computerHoles[i] = new JButton(""+(i));
            computerHoles[i].setEnabled(false);
            computerHoles[i].addActionListener(new ActionListener() { 
                @Override
                public void actionPerformed(ActionEvent e) {
                    ...
                    if(confirmMove == JOptionPane.YES_OPTION) {
                        ...
                        int index = Integer.parseInt(e.getActionCommand()) + 9;
                        if(!playTheGame.board.moveTheBalls(index)){
    ...
    private void updateGUI(){ 
        for(int i =0; i< MAX_HOLES; i++){
            ...
            cLabel[i].setText("Rocks: " + playTheGame.board.getBallsFromHole(i + 9));
        } 
